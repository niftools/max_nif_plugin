...
extern const EnumLookupType BSShaderTypes[] = { 
   { 0, _T( "Default" ) },
   { 1, _T( "BSShaderPPLightingProperty" ) },
   { 2, _T( "BSShaderNoLightingProperty" ) },
   { 3, _T( "WaterShaderProperty" ) },
   { 4, _T( "SkyShaderProperty" ) },
   { 7, _T( "TallGrassShaderProperty" ) },
   {10, _T( "Lighting30ShaderProperty" ) },
   {100, _T( "SkyrimDefault" ) },
   {101, _T( "SkyrimEnvMap" ) },
   {102, _T( "SkyrimGlow" ) },
   {105, _T( "SkyrimSkin" ) },
   {106, _T( "SkyrimHair" ) },
   {111, _T( "SkyrimParallax" ) },
   {115, _T( "SkyrimEye" ) },
   {-1, NULL },
};
...
public:
	...
	BOOL						KeyAtTime( int id, TimeValue t ) { return pb->KeyFrameAtTimeByID( id, t ); }
	...
	void						SetReference( int i, RefTargetHandle rtarg ) { if( i == 0 ) pb = (IParamBlock2*) rtarg; else assert( 0 ); }
...
public:
	...
	const TCHAR*	InternalName( ) { return _T( "NifShader" ); }
...
public:
	...
	void			DeleteThis( ) { delete this; }
	int				FindSubTexFromHWND( HWND hw ) { return -1; }
	...
	void SetTime( TimeValue t )
	{
		...
		if( !pShader->ivalid.InInterval( t ) )
		{
			...
			UpdateOpacity( ); // always update opacity since it's not int validity computations
	}
...
{
	pb = NULL;
	NifShaderDesc.MakeAutoParamBlocks( this ); // make and initialize paramblock2
	pDlg = NULL;
...
void NifShader::Reset( )
{
	...
	// don't want to see this parameter reset in macroreader
	macroRecorder->Disable( ); 
	...
	// Old code, not going to bother converting ...
...
void NifShaderCombineComponents( ShadeContext& sc, IllumParams& ip )
{
	...
	ip.finalC = o * ( ip.ambIllumOut + ip.diffIllumOut ) + ip.specIllumOut + ip.reflIllumOut + ip.transIllumOut;
}
...
void NifShader::Illum( ShadeContext& sc, IllumParams& ip )
{
	...
	for( int i = 0; i < sc.nLights; i++ )
	{
		...
		if( l->Illuminate( sc, sc.Normal( ), lightClr, L, NL, Kl ) )
		{
			...
			if( l->affectSpecular )
			{
				// strauss uses the reflect LIGHT vector
	...
	{
		// Set up attenuation opacity for Refraction map. dim diffue & spec by this
		ip.finalAttenuation = ip.finalOpac * ( 1.0f - ip.reflectAmt );
...
ShaderParamDlg* NifShader::CreateParamDialog( HWND hOldRollup, HWND hwMtlEdit, IMtlParams* imp, StdMat2* theMtl, int rollupOpen, int )
{
	...
	if( !hOldRollup )
	{
		pDlg->hRollup = imp->AddRollupPage( hInstance, MAKEINTRESOURCE( IDD_NIFSHADER ),
			NifShaderDlg::DlgProc, GetString( IDS_NIF_SHADER_BASIC ), (LPARAM) pDlg,
			rollupflags
			);
	}
	else
...
NifShaderDlg::NifShaderDlg( HWND hwMtlEdit, IMtlParams* pParams )
{
	...
	texMButDiffuse = NULL;
	pShininessSpinner = NULL;
	pAlphaSpinner = NULL;
	pParallaxOffsetSpinner = NULL;
	pBumpMagnitudeSpinner = NULL;
	pTestRefSpinner = NULL;
}
...
INT_PTR NifShaderDlg::PanelProc( HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	case CC_COLOR_BUTTONUP:
		if( code ) theHold.Accept( GetString( IDS_DS_PARAMCHG ) );
		else theHold.Cancel( );
		UpdateMtlDisplay( );
		break;
	...
	case CC_SPINNER_BUTTONUP:
		if( code || msg == WM_CUSTEDIT_ENTER ) theHold.Accept( GetString( IDS_DS_PARAMCHG ) );
		else theHold.Cancel( );
