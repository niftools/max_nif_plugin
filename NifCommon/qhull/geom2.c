...
void qh_detroundoff (void) {
  ...
  qh MINdenom_1_2= qsqrt (qh MINdenom_1 * qh hull_dim) ;  /* if will be normalized */
  ...
  { /* compute ONEmerge, max vertex offset for merging simplicial facets */
    ...
    qh ONEmerge= qsqrt (qh hull_dim) * qh MAXwidth *
      qsqrt (1.0 - maxangle * maxangle) + qh DISTround;  
  ...
  if (qh JOGGLEmax < REALmax/2 && (qh KEEPcoplanar || qh KEEPinside)) {
    ...
    maxdist= qsqrt (qh hull_dim) * qh JOGGLEmax + qh DISTround;
...
/*-<a                             href="qh-geom.htm#TOC"
  ...
  notes:
    ...
    use qsqrt(dim) since one vector is normalized
      or use maxsumabs since one vector is < 1
*/
realT qh_distround (int dimension, realT maxabs, realT maxsumabs) {
  realT maxdistsum, maxround;

  maxdistsum= qsqrt (dimension) * maxabs;
...
boolT qh_gram_schmidt(int dim, realT **row) {
  ...
  for(i=0; i < dim; i++) {
    ...
    if (norm == 0.0)  /* either 0 or overflow due to qsqrt */
      return False;
    for(k= dim; k--; )
      *(--rowi) /= norm;  
    for(j= i+1; j < dim; j++) {
...
void qh_outerinner (facetT *facet, realT *outerplane, realT *innerplane) {
  ...
  if (outerplane) {
    ...
    if (qh JOGGLEmax < REALmax/2)
      *outerplane += qh JOGGLEmax * qsqrt (qh hull_dim);
  }
  if (innerplane) {
    if (facet) {
      mindist= REALmax;
    ...
    if (qh JOGGLEmax < REALmax/2)
      *innerplane -= qh JOGGLEmax * qsqrt (qh hull_dim);
  }
} /* outerinner */
